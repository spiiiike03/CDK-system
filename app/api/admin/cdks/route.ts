...
import { generateCdk } from "@/lib/cdk";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileCount = Math.min(100, Math.max(1, Math.floor(Number(body.fileCount) || 1)));
    const maxUses = Math.min(100, Math.max(1, Math.floor(Number(body.maxUses) || 1)));
    const prefix = String(body.prefix || "CDK");
    ...
    for (let index = 0; index < quantity; index += 1) {
      ...
      for (let attempt = 0; attempt < 5 && !inserted; attempt += 1) {
        ...
        const rows = await sql`
          ...
          values (${code}, ${fileCount}, ${maxUses}, ${expiresAt})
